...
| Our Governance | ✅ Done | Honey-bordered pillar cards, rust committee badges (`b6cbd56`). Re-checked node 468-1448 — content/cards already matched; fixed one background seam (pillars section was `bg-tan`, Figma shows one continuous backdrop across pillars + committees) by switching to `bg-cream` (`0a8e623`). That flattened the seam but made the white pillar cards nearly indistinguishable from the section background (cream `#faf6ef` vs. white `#ffffff` — too close in value, cards read as "transparent"), so both sections were unified on `bg-tan` instead — still one continuous background, but with real contrast for the white cards. Left the faint decorative line/swirl texture behind those sections unreplicated — it's hand-drawn Figma art with no equivalent asset anywhere else on the site, so didn't fabricate it; flag if you want it added. Also rebuilt the hero as a 2-column layout (text left, gradient-block right) to match Figma's structure — previously kept as centered text-only; now uses the same gradient-placeholder treatment as the DisciplinePage/LocationPage hero pattern in place of Figma's "Photography placeholder" (`<pending commit>`). |
